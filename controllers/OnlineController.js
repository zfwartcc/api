...
router.get('/top', async (req, res) => {
	try {
		...
		const sessions = await ControllerHours.find({timeStart: {$gt: thisMonth, $lt: nextMonth}, position: { $not: /.*_(I|M)_.*/, $or: [{ position: "ORD_I_GND" }, { position: "ORD_M_GND" }]}}).populate('user', 'fname lname cid');
